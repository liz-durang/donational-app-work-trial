...
RSpec.describe Payments::RefundPaymentIntent do
  ...
  context 'when the payment intent ID and account ID are supplied' do
    ...
    context 'and the stripe response is successful' do
      xit 'refunds the payment intent' do
        command = described_class.run(payment_intent_id: payment_intent.id, application_fee_amount_cents: 40)
    ...
    context 'and the payment intent ID is invalid' do
      ...
      it 'fails with errors' do
        ...
        command = described_class.run(payment_intent_id: 'invalid ID', application_fee_amount_cents: 40, account_id:)
    ...
    context 'when the application fee amount is not supplied' do
      let(:application_fee_amount_cents) { nil }

      it 'fails with errors' do
        command = described_class.run(payment_intent_id: payment_intent.id, application_fee_amount_cents:)
  ...
  context 'when the payment intent ID is not supplied' do
    ...
    it 'fails with errors' do
      command = described_class.run(payment_intent_id:, application_fee_amount_cents: 40)
