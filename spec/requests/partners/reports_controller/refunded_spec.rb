...
RSpec.describe 'GET /partners/:partner_id/reports/refunds', type: :request do
  ...
  before do
    ...
    allow(Partners::GetRefundedExport).to receive(:call).and_return(OpenStruct.new(to_sql: 'SELECT 1'))
  ...
  context 'when the donor has permission' do
    it 'returns a successful response' do
      get refunded_partner_reports_path(partner, format: :csv,
                                                  params: { start_at: start_date.to_s, end_at: end_date.to_s })
      expect(response).to have_http_status(:success)
    end

    it 'calls the GetRefundedExport command' do
      expect(Partners::GetRefundedExport).to receive(:call).with(partner:, donated_between:)
      get refunded_partner_reports_path(partner, format: :csv,
                                                  params: { start_at: start_date.to_s, end_at: end_date.to_s })
    end
