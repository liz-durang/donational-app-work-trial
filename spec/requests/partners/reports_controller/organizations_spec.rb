...
RSpec.describe 'GET /partners/:partner_id/reports/organizations', type: :request do
  ...
  let(:start_date) { (Date.today - 1.month).in_time_zone('Eastern Time (US & Canada)') }
  ...
  before do
    ...
    allow(Partners::GetOrganizationsExport).to receive(:call).and_return(OpenStruct.new(to_sql: 'SELECT 1'))
  ...
  context 'when the donor has permission' do
    it 'returns a successful response' do
      get organizations_partner_reports_path(partner, format: :csv,
                                                      params: { start_at: start_date.to_s, end_at: end_date.to_s })
      expect(response).to have_http_status(:success)
    end

    it 'calls the GetOrganizationsExport command' do
      expect(Partners::GetOrganizationsExport).to receive(:call).with(partner:,
                                                                      donated_between:)
      get organizations_partner_reports_path(partner, format: :csv,
                                                      params: { start_at: start_date.to_s, end_at: end_date.to_s })
    end
    ...
    it 'calls stream_sql_data_as_csv with the correct arguments' do
      expected_filename = "#{partner.name.parameterize}-organizations-#{start_date.to_date.iso8601}-to-#{end_date.to_date.iso8601}.csv"
