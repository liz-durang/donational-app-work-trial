class TriggerRecurringContributionCreatedWebhook < ApplicationJob
  ...
  def perform(recurring_contribution_id, partner_id)
    ...
    if ensure_partner_has_webhook(current_partner)
      ...
      base_url = current_partner.zapier_webhooks.find_by(hook_type: 'create_recurring_contribution').hook_url
  ...
  def ensure_partner_has_webhook(current_partner)
      Hooks::GetZapierWebhookByType.call(partner: current_partner, hook_type: 'create_recurring_contribution').present?
  end
end
